...
for s in Pilot04; do
	...
	for task in HTBMB; do
		...
		for run in 001 002 003 004 005 006 007 008; do
			...
			3dmerge -1blur_fwhm 4.0 -doall -prefix ${Raw}/func/sub-${s}_task-${task}_run-${run}_bold_space-T1w_smoothed_preproc.nii.gz \
			${Raw}/func/sub-${s}_task-${task}_run-${run}_bold_space-T1w_preproc.nii.gz
			#3dBlurToFWHM -input ${Raw}/func/sub-${s}_task-${task}_run-${run}_bold_space-T1w_preproc.nii.gz \
			#-prefix ${Raw}/func/sub-${s}_task-${task}_run-${run}_bold_space-T1w_smoothed_preproc.nii.gz \
			#-FWHM 6		

			#scaling
			3dTstat -mean -prefix ${Raw}/func/sub-${s}_task-${task}_run-${run}_bold_space-T1w_mean.nii.gz \
			${Raw}/func/sub-${s}_task-${task}_run-${run}_bold_space-T1w_smoothed_preproc.nii.gz

			3dcalc \
			-a ${Raw}/func/sub-${s}_task-${task}_run-${run}_bold_space-T1w_smoothed_preproc.nii.gz \
			-b ${Raw}/func/sub-${s}_task-${task}_run-${run}_bold_space-T1w_mean.nii.gz \
			-c ${Raw}/func/sub-${s}_task-${task}_run-${run}_bold_space-T1w_brainmask.nii.gz\
			-expr "(a/b * 100) * c" \
			-prefix ${Raw}/func/sub-${s}_task-${task}_run-${run}_bold_space-T1w_smoothed_scaled_preproc.nii.gz

			#remove not needed files
			rm ${Raw}/func/sub-${s}_task-${task}_run-${run}_bold_space-T1w_mean.nii.gz
			rm ${Raw}/func/sub-${s}_task-${task}_run-${run}_bold_space-T1w_smoothed_preproc.nii.gz
		done	
		...
		3dMean -count -prefix ${Output}/sub-${s}/union_mask.nii.gz ${Raw}/func/*task-${task}*T1w_brainmask.nii.gz


		3dDeconvolve -input $(/bin/ls ${Raw}/func/sub-${s}_task-${task}_run-*_bold_space-T1w_smoothed_scaled_preproc.nii.gz | sort -V) \
		...
		-rout \
		-tout \
		-bucket ${Output}/sub-${s}/FIRmodel_task-${task}_stats.nii.gz \
		-GOFORIT 100 \
		-noFDR \
		-nocout \
		-allzero_OK
